...
export function navigateToObject(
  ...
  let floor = findObjectFloor(selectedObjectId)?.floor || "ground";
  ...
  if (!target) {
    ...
    return selectedFloor; // no change
  }
...
export function navigateWithDelay(
  ...
  if (index < objects.length) {
    ...
    const newFloor = navigateToObject(obj.name, navigation, setNavigation, selectedFloor, setSelectedFloor);

    setTimeout(() => {
      navigateWithDelay(objects, index + 1, delay, navigation, setNavigation, newFloor, setSelectedFloor);
    }, delay);
  }
}
